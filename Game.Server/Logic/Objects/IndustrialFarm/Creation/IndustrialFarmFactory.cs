        ...
        public GameObjectAggregator CreateNew(Coordiante root, Coordiante[] area, int player)
        {
            return new GameObjectAggregatorBuilder(BuildingTypes.IndustrialFarm, player)
                ...
                .AsManufactoring(new ManufactoringArgs
                {
                    ...
                    RequriedResources = new[] { ResourceChunk.Create(ResourceType.Energy, 1), ResourceChunk.Create(ResourceType.Chemicals, 0.1f) },
                    ResultResources = new[] { ResourceChunk.Create(ResourceType.Food, 2) }
                })
